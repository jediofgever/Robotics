...
def path(grid, h, w, goal, gv, init, actions, costs):
    value = [[99 for col in range(w)] for row in range(h)]
    change = True
    ...
    while change:
        ...
        for x in range(h):
            for y in range(w):
                ...
                elif (not grid[x][y] or grid[x][y] == 'x'):
                    for a in range(len(actions)):
                        dx, dy = actions[a]
                        x2 = x + dx 
                        y2 = y + dy

                        if x2 >= 0 and x2 < len(grid) and y2 >= 0 and y2 < len(grid[0]) and (not grid[x2][y2] or grid[x2][y2] == gv):
...
def plan(warehouse, dropzone, todo):
    ...
    # find grid position for each todo item
    todo_positions = {}
    for row in range(h):
        for col in range(w):
            value = warehouse[row][col]
            if value in todo:
                todo_positions[value] = (row, col)

    assert(len(todo_positions) == len(todo))                

    cost = 0.0
    for item in todo:
        pos = todo_positions[item]
        cost += path(warehouse, h, w, pos, item, dropzone, actions, costs) 
        warehouse[pos[0]][pos[1]] = 0 
        cost += path(warehouse, h, w, dropzone, 'x', pos, actions, costs) 
    return cost
