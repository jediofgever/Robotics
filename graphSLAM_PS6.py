...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    omega = [[0.0 for idx in range(size)] for jdx in range(size)]
    xi = [[0.0] for idx in range(size)]
    ...
    for step in range(len(data)):
        ...
        omega[cstep][cstep] += motion_sigma
        omega[cstep][fstep] -= motion_sigma
        xi[cstep][0] -= (dx / motion_noise)

        omega[cstep+1][cstep+1] += motion_sigma
        omega[cstep+1][fstep+1] -= motion_sigma
        xi[cstep+1][0] -= (dy / motion_noise)

        omega[fstep][cstep] -= motion_sigma
        omega[fstep][fstep] += motion_sigma
        xi[fstep][0] += (dx / motion_noise)

        omega[fstep+1][cstep+1] -= motion_sigma
        omega[fstep+1][fstep+1] += motion_sigma
        xi[fstep+1][0] += (dy / motion_noise)

        for landmark in Z:
            idx, lx, ly = landmark
            offset = 2*(N+idx)

            omega[cstep][cstep] += measurement_sigma
            omega[cstep][offset] -= measurement_sigma
            xi[cstep][0] -= (lx / measurement_noise)

            omega[cstep+1][cstep+1] += measurement_sigma
            omega[cstep+1][offset+1] -= measurement_sigma
            xi[cstep+1][0] -= (ly / measurement_noise)

            omega[offset][cstep] -= measurement_sigma
            omega[offset][offset] += measurement_sigma
            xi[offset][0] += (lx / measurement_noise)

            omega[offset+1][cstep+1] -= measurement_sigma
            omega[offset+1][offset+1] += measurement_sigma
            xi[offset+1][0] += (ly / measurement_noise)
