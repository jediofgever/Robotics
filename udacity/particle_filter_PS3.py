...
class robot:
    ...
    # move: 
    #   
    def move(self, motion, epsilon=1e-3):
        result = robot()
        ...
        steering_angle, distance = motion
        turning_angle = random.gauss(steering_angle, self.steering_noise)
        d = random.gauss(distance, self.distance_noise)
        beta = (d / self.length) * tan(turning_angle)
    ...
    def sense(self, add_noise=1):
        ...
        for landmark in landmarks:
            ...
            delta_y = y - self.y
            delta_x = x - self.x
            landmark_orientation = (atan2(delta_y, delta_x) - self.orientation)
            if add_noise:
                noise = random.gauss(0.0, self.bearing_noise)
                landmark_orientation += noise
            landmark_orientation %= (2.0 * pi)
            Z.append(landmark_orientation)
        return Z
