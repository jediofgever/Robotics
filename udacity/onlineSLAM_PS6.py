...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    motion_sigma = 1.0 / motion_noise
    measurement_sigma = 1.0 / measurement_noise

    size = 2 * (1 + num_landmarks)
    new_size = 2 * (2 + num_landmarks)
    omega = matrix([[0.0 for idx in range(size)] for jdx in range(size)])
    xi = matrix([[0.0] for idx in range(size)])
    ...
    for step in range(len(data)):
        ...
        # 1) Expand
        # x_t, x_t+1, L0, ... , LN
        expand = [idx+2 if idx >= 2 else idx for idx in range(size)]
        omega = omega.expand(new_size, new_size, expand, expand)
        xi = xi.expand(new_size, 1, expand, [0])
        ...
        # Motion Update
        omega.value[cstep][cstep] += motion_sigma
        omega.value[cstep][fstep] -= motion_sigma
        xi.value[cstep][0] -= (dx / motion_noise)

        omega.value[cstep+1][cstep+1] += motion_sigma
        omega.value[cstep+1][fstep+1] -= motion_sigma
        xi.value[cstep+1][0] -= (dy / motion_noise)

        omega.value[fstep][cstep] -= motion_sigma
        omega.value[fstep][fstep] += motion_sigma
        xi.value[fstep][0] += (dx / motion_noise)

        omega.value[fstep+1][cstep+1] -= motion_sigma
        omega.value[fstep+1][fstep+1] += motion_sigma
        xi.value[fstep+1][0] += (dy / motion_noise)
        ...
        for landmark in Z:
            ...
            omega.value[cstep][cstep] += measurement_sigma
            omega.value[cstep][offset] -= measurement_sigma
            xi.value[cstep][0] -= (lx / measurement_noise)

            omega.value[cstep+1][cstep+1] += measurement_sigma
            omega.value[cstep+1][offset+1] -= measurement_sigma
            xi.value[cstep+1][0] -= (ly / measurement_noise)

            omega.value[offset][cstep] -= measurement_sigma
            omega.value[offset][offset] += measurement_sigma
            xi.value[offset][0] += (lx / measurement_noise)

            omega.value[offset+1][cstep+1] -= measurement_sigma
            omega.value[offset+1][offset+1] += measurement_sigma
            xi.value[offset+1][0] += (ly / measurement_noise)

        # 3) Take
        small = [0,1]
        large = [idx for idx in range(2, new_size, 1)]
        A = omega.take(small, large)
        B = omega.take(small, small)
        C = xi.take(small, [0])
        omega1 = omega.take(large, large)
        xi1 = xi.take(large, [0])

        # 4) New omega, xi
        omega = omega1 - A.transpose() * B.inverse() * A
        xi = xi1 - A.transpose() * B.inverse() * C
